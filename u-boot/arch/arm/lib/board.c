...
extern int recovery_notcheck_display;
extern int update_env(void);
#endif
...
//	printf("sunxi script init\n");
	...
	else if(1 == storage_type || 0 == storage_type){
		mmc_card_no = 0;
	}
	else{
		mmc_card_no = 2;
...
/* add by cjcheng for display_param start ...*/
#ifdef CONFIG_AUTO_UPDATE_ENV
	if (recovery_notcheck_display == 0) {
		if (update_env()){
			printf("update smdt param error\n");
		}else{
			printf("update smdt param success\n");
		}
	}
#endif
/* add by cjcheng for display_param end ...*/
