...
#include <mach/sys_config.h>
#include <asm/io.h>
//#include "../misc/sw_3g_module/sw_module.h"
...
void gpio_dldo_on_off(struct sw_gpio *modem, u32 on);
...
void gpio_dldo_on_off(struct sw_gpio *modem, u32 on)
{
    ...
    if(modem->dldo_name[0] == 0 || modem->dldo_min_uV == 0 || modem->dldo_max_uV == 0){
        printk("err: dldo parameter is invalid. dldo_name=%s, dldo_min_uV=%d, dldo_max_uV=%d.\n",
...
static int __init gpio_sw_init(void)
{
	...
	//------------------PL ldo ------------------
	/* pinl_dldo_min_uV */
	type = script_get_item("gpio_para", "pinl_dldo_min_uV", &val);
	if(type == SCIRPT_ITEM_VALUE_TYPE_INT){
	    modem.dldo_min_uV = val.val;
	}else{
	    printk("ERR: get bb_dldo_min_uV failed\n");
	    modem.dldo_min_uV = 0;
	}

	/* pinl_dldo_max_uV */
	type = script_get_item("gpio_para", "pinl_dldo_max_uV", &val);
	if(type == SCIRPT_ITEM_VALUE_TYPE_INT){
	    modem.dldo_max_uV = val.val;
	}else{
	    printk("ERR: get bb_dldo_max_uV failed\n");
	    modem.dldo_max_uV = 0;
	}

        /* pinl_dldo */
	type = script_get_item("gpio_para", "pinl_dldo", &val);
	if(type == SCIRPT_ITEM_VALUE_TYPE_STR){
	    strcpy(modem.dldo_name, val.str);
	    printk("%s modem support\n", modem.dldo_name);
	    gpio_dldo_on_off(&modem,1);
	}else{
	    printk("ERR: get gpio dldo_name failed\n");
	}

	//------------------PB ldo ------------------
	/* pinb_dldo_min_uV */
	type = script_get_item("gpio_para", "pinb_dldo_min_uV", &val);
	if(type == SCIRPT_ITEM_VALUE_TYPE_INT){
	    modem.dldo_min_uV = val.val;
	}else{
	    printk("ERR: get bb_dldo_min_uV failed\n");
	...
	/* pinb_dldo_max_uV */
	type = script_get_item("gpio_para", "pinb_dldo_max_uV", &val);
	if(type == SCIRPT_ITEM_VALUE_TYPE_INT){
	    modem.dldo_max_uV = val.val;
	}else{
	    printk("ERR: get bb_dldo_max_uV failed\n");
        ...
        /* pinb_dldo */
	type = script_get_item("gpio_para", "pinb_dldo", &val);
	if(type == SCIRPT_ITEM_VALUE_TYPE_STR){
	    strcpy(modem.dldo_name, val.str);
	    printk("%s modem support\n", modem.dldo_name);
	    gpio_dldo_on_off(&modem,1);
	...
	//------------------PG ldo ------------------
	/* ping_dldo_min_uV */
	type = script_get_item("gpio_para", "ping_dldo_min_uV", &val);
	if(type == SCIRPT_ITEM_VALUE_TYPE_INT){
	    modem.dldo_min_uV = val.val;
	}else{
	    printk("ERR: get bb_dldo_min_uV failed\n");
	...
	/* ping_dldo_max_uV */
	type = script_get_item("gpio_para", "ping_dldo_max_uV", &val);
	if(type == SCIRPT_ITEM_VALUE_TYPE_INT){
	    modem.dldo_max_uV = val.val;
	}else{
	    printk("ERR: get bb_dldo_max_uV failed\n");
        ...
        /* ping_dldo */
	type = script_get_item("gpio_para", "ping_dldo", &val);
	if(type == SCIRPT_ITEM_VALUE_TYPE_STR){
	    strcpy(modem.dldo_name, val.str);
	    printk("%s modem support\n", modem.dldo_name);
	    gpio_dldo_on_off(&modem,1);
