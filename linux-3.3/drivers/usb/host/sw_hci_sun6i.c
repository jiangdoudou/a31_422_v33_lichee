...
static void sw_usb_3g_config(struct sw_hci_hcd *sw_hci)
{
	...
	struct module_pio bb_pwr_on;
	struct module_pio bb_rst;	
	int ret = 0;

	#if 1
	/* bb_pwr_on */
    type = script_get_item("3g_para", "bb_pwr_on", &bb_pwr_on.pio);
    if(type == SCIRPT_ITEM_VALUE_TYPE_PIO){
        bb_pwr_on.valid = 1;
    }else{
        printk("ERR: get bb_pwr_on failed\n");
        bb_pwr_on.valid  = 0;
    }

    /* bb_rst */
    type = script_get_item("3g_para", "bb_rst", &bb_rst.pio);
    if(type == SCIRPT_ITEM_VALUE_TYPE_PIO){
        bb_rst.valid = 1;
    }else{
        printk("ERR: get bb_rst failed\n");
        bb_rst.valid  = 0;
    }
	...
	//---------------------------------
	   ...
	   //---------------------------------
	   if(bb_pwr_on.valid){
		   ret = gpio_request(bb_pwr_on.pio.gpio.gpio, "bb_pwr_on");
		   if(ret != 0){
			   printk("gpio_request bb_pwr_on failed\n");
			   bb_pwr_on.valid = 0;
		   }else{
			   /* set config, ouput */
			   sw_gpio_setcfg(bb_pwr_on.pio.gpio.gpio, GPIO_CFG_OUTPUT);
		   }
	   }
	   ...
	   //---------------------------------
	   if(bb_rst.valid){
		   ret = gpio_request(bb_rst.pio.gpio.gpio, "bb_rst");
		   if(ret != 0){
			   printk("gpio_request bb_rst failed\n");
			   bb_rst.valid = 0;
		   }else{
			   /* set config, ouput */
			   sw_gpio_setcfg(bb_rst.pio.gpio.gpio, GPIO_CFG_OUTPUT);
		   }
	   }
     ...
     __gpio_set_value(bb_rst.pio.gpio.gpio, 1);	
    	
		if(!bb_pwr_on.valid){
          return;
        }
       __gpio_set_value(bb_pwr_on.pio.gpio.gpio, 1);
    ...
    __gpio_set_value(bb_pwr_on.pio.gpio.gpio, 0);	
