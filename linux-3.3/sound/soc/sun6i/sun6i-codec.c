...
static int codec_pa_play_open(void)
{
	...
	printk("codec_pa_play_open: enter\n");
...
static int codec_headphone_play_open(void)
{
	...
	printk("codec_headphone_play_open: enter\n");
	...
	printk("init volume: %d\n", reg_val);
	if (!reg_val) {
		printk("increase Volume to %d\n", headphone_vol);
	...
	printk("codec_headphone_play_open: end\n");
	return 0;
}
...
static int codec_earpiece_play_open(void)
{
	...
	printk("codec_earpiece_play_open: enter\n");
	...
	printk("codec_earpiece_play_open: end\n");
...
static int codec_pa_and_headset_play_open(void)
{
	...
	printk("codec_pa_and_headset_play_open: enter\n");
	...
	printk("codec_pa_and_headset_play_open: end\n");
...
{
	printk("codec_voice_main_mic_capture_open: enter\n");
...
static int codec_play_start(void)
{
	...
	if (codec_speaker_en || (codec_speaker_headset_earpiece_en==1)||(codec_speaker_headset_earpiece_en==2)) {
		...
		printk("set the default output is HPOUTL-R\n");
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	printk("codec_set_speakerout_lntor: enter\n");
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	printk("codec_set_speakerout: enter\n");
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	printk("switch to headphone: %d\n", codec_headphoneout_en);
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	printk("switch earpicece: %d\n", codec_earpieceout_en);
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	printk("codec_set_mainmic: enter\n");
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	//printk("codec_set_spk: enter\n");

	codec_speaker_en = ucontrol->value.integer.value[0];
	printk("switch speaker/headphone: %d\n",codec_speaker_en);
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	printk("codec_set_spk_headset_earpiece: enter\n");

	codec_speaker_headset_earpiece_en = ucontrol->value.integer.value[0];
	
	printk("codec_speaker_headset_earpiece_en: %d\n", codec_speaker_headset_earpiece_en);
...
static int snd_sun6i_codec_prepare(struct snd_pcm_substream	*substream)
{
	...
	printk("snd_sun6i_codec_prepare: enter\n");
		...
		capture_prtd = substream->runtime->private_data;
	  	 ...
	  	 * codec <--> BT codec or GSM modem -- lg FIXME */
	   	...
	   	if (codec_adcphonein_en) {
			...
			printk("codec_voice_main_mic_capture_open ..\n");
...
static void spk_vol_work(struct work_struct *work)
{
	...
	ret = axp_spk_det();
	if (ret == 1) {
		codec_wr_control(SUN6I_MIC_CTRL, 0x1f, LINEOUT_VOL, 0x1b);
	} else {
