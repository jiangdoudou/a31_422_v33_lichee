...
#include <linux/init.h> // add by cjcheng for display_param
#include <linux/ctype.h> // add by cjcheng for display_param
...
extern char smdt_cmd_line[]; // add by cjcheng for display_param
...
script_get_item(char *main_key, char *sub_key, script_item_u *item)
{
    ...
    int smdt_param = -1; // add by cjcheng for display_param
    ...
    while(mainkey) {
        if((mainkey->hash == main_hash) && !strcmp(mainkey->name, main_key)) {
            ...
            smdt_param = -1; // add by cjcheng for display_param
